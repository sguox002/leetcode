...
vector<int> findSubstring(string s, vector& words) {
	...
	vector<int> indexes;
	for (int i = 0; i < n - num * len + 1; i++) { //sliding window 
...
```
this is amazingly short solution and is also pretty straightforward.


### 68	Text Justification	 	Hard	
given max width in a line. pad extra space from left to right.
last line shall be left adjusted.
...
vector<string> fullJustify(vector<string> &words, int L) {
    ...
    for(int i = 0; i < words.size(); i += k) {
		int k=0,l=0;
        for(k = l = 0; i + k < words.size() and l + words[i+k].size() <= L - k; k++) { //leave k spaces
            l += words[i+k].size();
        }
        string tmp = words[i];
        for(int j = 0; j < k - 1; j++) {
