...
```cpp
    ...
    int findGoodStrings(int n, string s1, string s2, string evil) {
		bool bb=s2.find(evil)!=string::npos;
        int a=less(s2,0),b=less(s1,0),c=evil_less(s2,evil,0),d=evil_less(s1,evil,0);
        cout<<a<<" "<<b<<" "<<c<<" "<<d<<endl;
        return less(s2,0)-less(s1,0)+1-(evil_less(s2,evil,0)-evil_less(s1,evil,0)+bb);
    }
	int less(string s,int start){
		if(start>=s.size()) return 0;
		int t=s[start]-'a'; //number of less 
		int n=s.size();
		return (t*mypow(26,n-start-1)%mod+less(s,start+1))%mod;
	}
	int evil_less(string s,string& evil,int start){ //find all string with evil as substring
		int n=s.size(),m=evil.size();
		if(n<m) return 0;
		if(start>n-m) return 1;
		//evil>substr, prefix must be less, and postfix don't care.
		//evil==substr, prefix less+postfix don't care, prefix= and postfix less
		//evil<substr, pefix less+postfix don't care, prefix= and postfix don't care.
		string t=s.substr(start,m);
		int ans=evil_less(s.substr(0,start),evil,0)*(mypow(26,n-m-start)-evil_less(s.substr(start+m),evil,0))%mod;
		if(evil==t) ans+=evil_less(s.substr(start+m),evil,0);
		if(evil<t) ans+=evil_less(s.substr(start+m),evil,0)%mod;
        ans+=evil_less(s,evil,start+1);
		return ans%mod;
	}
	
	int mypow(int a,int n){
		int ans=1;
		while(n--) ans*=a,ans%=mod;
		return ans;
	}
```	
